...
class listener implements EventSubscriberInterface
{
	...
	* @param \phpbb\cache\driver\driver_interface	$cache		Cache service
	...
	*/
	public function index_modify_page_title($event)
	{
		...
		if ($anniversary_data === false || $time > $this->config['vinabb_happyanniversaryemail_check_gc'])
		{
			// Get the begin year
			...
			if (sizeof($anniversary_data) && $time > $this->config['vinabb_happyanniversaryemail_check_gc'])
			{
		...
		foreach ($anniversary_data as $user_id => $user_data)
		{
			$anniversary_users .= (empty($anniversary_users) ? '' : ', ') . get_username_string('full', $user_id, $user_data['username'], $user_data['color']) . ' (' . $user_data['years'] . ')';
		}
